#!/usr/bin/env python3
"""
Copy PostGIS Files to PostgreSQL 17 Extension Directory

This script copies PostGIS extension files from the installed directory
to the PostgreSQL 17 extension directory so PostgreSQL can find them.
"""
...
def copy_postgis_to_postgres17():
    """Copy PostGIS files to PostgreSQL 17 extension directory"""
    
    print("🚀 Copying PostGIS Files to PostgreSQL 17 Extension Directory")
    print("=" * 75)
    
    # Source directory (where PostGIS is installed)
    postgis_source = Path("C:/Program Files/postgis-3.6.0rc2/postgis-3.6.0rc2")
    
    # Target directory (PostgreSQL 17 extension directory)
    postgres_target = Path("C:/Program Files/PostgreSQL/17/share/extension")
    ...
    if not postgres_target.exists():
        print(f"❌ PostgreSQL 17 extension directory not found: {postgres_target}")
        print("💡 Please check the PostgreSQL 17 installation path")
        return False
    ...
    for source_file in extension_files:
        try:
            ...
            if "Permission denied" in str(e):
                print(f"      💡 Try running PowerShell as Administrator")
    ...
    if copied_count > 0:
        ...
        print(f"💡 You may need to restart PostgreSQL 17 service for changes to take effect")
...
if __name__ == "__main__":
    success = copy_postgis_to_postgres17()
