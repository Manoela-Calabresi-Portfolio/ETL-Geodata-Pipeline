...
def get_next_run_number():
    ...
    existing_runs = [d for d in base_dir.iterdir() 
                    if d.is_dir() and d.name.startswith("stuttgart_enhanced_analysis_")]
...
OUT_DIR = Path(f"outputs/stuttgart_enhanced_analysis_{RUN_NUMBER:03d}")
...
def generate_overview_maps(data):
    ...
    # Add 5% padding to the extent for better visual balance
    x_padding = (extent[2] - extent[0]) * 0.05
    y_padding = (extent[3] - extent[1]) * 0.05
    extent = (extent[0] - x_padding, extent[1] - y_padding, 
              extent[2] + x_padding, extent[3] + y_padding)
    
    # Map 1: Land use + Roads + PT stops
    fig, ax = plt.subplots(1, 1, figsize=(29, 19))  # 20% larger for more white margins
    ...
    ax.set_axis_off()
    # Main title in English - Acumin Bold 14
    ax.set_title("Stuttgart — Land Use + Roads + PT Stops", fontsize=14, fontweight="bold", 
                 fontfamily="Acumin Pro", pad=20)
    
    # German subtitle below - Acumin Italic 12 (closer to English title)
    ax.text(0.1, 0.92, "Stuttgart — Flächennutzung + Straßen + ÖPNV-Haltestellen", 
            transform=ax.transAxes, ha='center', fontsize=12, style='italic', 
            fontfamily="Acumin Pro", color="#333333")
    ...
    export_to_kepler(data, city_boundary_buffered, Path("outputs/maps"))
...
def _add_scale_bar(ax, extent):
    ...
    # Multiple scale bars: 500m, 1km, 2km, 5km - properly spaced
    scales = [
        (500, "500 m"),
        (1000, "1 km"), 
        (2000, "2 km"),
    ...
    for i, (length, label) in enumerate(scales):
        y_offset = scale_y - (i * 200)  # Increased spacing between scale bars
        ax.plot([scale_x, scale_x + length], [y_offset, y_offset], 'k-', linewidth=2)
        ax.text(scale_x + length/2, y_offset - 80, label, ha='center', va='top', 
...
def main():
    ...
    print(f"📊 KPIs: {kpi_csv_path}")
    print(f"🌐 Dashboard: {OUT_DIR}/stuttgart_report.html")
    print("\n💡 Open the HTML file in your browser to view the complete analysis!")
